---
name: policies
description: View organization security policies for AgentKeeper. Shows detection mode, blocked tools, blocked commands, path restrictions, and custom blocklists. Run when the user wants to see what policies are enforced, check security settings, or view org configuration.
---

# AgentKeeper Policies

You are displaying the organization's AgentKeeper security policies.
...
Running with default detection patterns. In local mode, AgentKeeper uses
...
curl -s --max-time 5 "https://www.agentkeeper.dev/api/v1/claude-code/policies" \
...
[ -n "$CLAUDE_PLUGIN_DATA" ] && AGENTKEEPER_DIR="$CLAUDE_PLUGIN_DATA"
API_KEY=$(cat "$AGENTKEEPER_DIR/api_key")
curl -s --max-time 5 "https://www.agentkeeper.dev/api/v1/shield/policies" \
  -H "Authorization: Bearer $API_KEY"
```
...
[ -n "$CLAUDE_PLUGIN_DATA" ] && AGENTKEEPER_DIR="$CLAUDE_PLUGIN_DATA"
API_KEY=$(cat "$AGENTKEEPER_DIR/api_key")
curl -s --max-time 5 "https://www.agentkeeper.dev/api/v1/claude-code/config" \
  -H "Authorization: Bearer $API_KEY"
```
...
https://www.agentkeeper.dev/pricing

[If plan is "pro" or "enterprise"]:
---
Manage policies at: https://www.agentkeeper.dev/dashboard/policies
```
...
Run /agentkeeper:connect to re-authenticate.
```
