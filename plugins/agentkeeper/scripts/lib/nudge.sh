...
get_nudge_text() {
  ...
  case "$trigger" in
    first_block)
      printf 'AgentKeeper caught %s. Running in local mode — connect your free account for dashboard visibility: /agentkeeper:connect' \
        "$pattern_name"
      ;;
    fifth_block)
      printf 'AgentKeeper has caught 5 threats. Your free dashboard tracks all of these with full context. /agentkeeper:connect'
      ;;
    session_start)
      printf 'AgentKeeper: protecting this session locally. Free dashboard available at www.agentkeeper.dev'
      ;;
    *)
      printf ''
      ;;
