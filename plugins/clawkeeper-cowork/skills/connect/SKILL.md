---
name: connect
description: Connect Clawkeeper to a free account for dashboard visibility and org policy sync. Run when the user wants to authenticate, link their account, set up an API key, or connect to clawkeeper.dev.
allowed-tools: Bash,Read
---

# Clawkeeper Connect

You are helping the user connect their Clawkeeper Cowork plugin to their clawkeeper.dev account using device authorization.

## Step 1: Check Existing Connection

First check if an API key already exists:
```bash
DATA_DIR="${CLAWKEEPER_DATA_DIR:-$HOME/.clawkeeper-cowork}"
cat "$DATA_DIR/api_key" 2>/dev/null
```
...
Your Clawkeeper security tools are syncing org policies.
Run /clawkeeper:audit to scan your project.
```
...
Your Clawkeeper security tools will now sync org policies.
