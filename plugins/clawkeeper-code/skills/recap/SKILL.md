...
Session tracking begins after the first tool call in a Clawkeeper-enabled
session. The Clawkeeper hooks record tool calls, file changes, and threat
...
In the meantime, try:
  /clawkeeper:audit — check your Claude Code security setup
  /clawkeeper:secrets — scan for exposed secrets in this directory
```
...
[If local mode]: Connect your free account to track sessions over time: /clawkeeper:connect
```
