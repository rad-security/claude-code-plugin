---
name: connect
description: Deprecated alias for /clawkeeper-code:login. Still functional — runs the same device-code flow — but prompts the user to switch to /login going forward.
---

# Clawkeeper Connect (deprecated alias for `login`)

Before doing anything else, display this one-line notice to the user:

```
Note: /clawkeeper-code:connect has been renamed to /clawkeeper-code:login.
