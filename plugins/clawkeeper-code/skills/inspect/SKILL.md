...
```
If found, check whether the URLs point to known/expected domains (like `clawkeeper.dev`, `anthropic.com`, `api.github.com`) vs unknown external URLs.
...
When reporting on the Clawkeeper plugin itself, note that it is the security scanner and its patterns (network calls to clawkeeper.dev, credential file checks) are expected behavior. Mark it with a note: "This is the Clawkeeper security plugin (self)".
...
- Skip the Clawkeeper plugin's own expected patterns but still report its structure for transparency
