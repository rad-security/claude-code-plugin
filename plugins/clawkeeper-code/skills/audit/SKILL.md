...
- Generic: `ck_live_` in non-example files
...
- **WARN**: Hook scripts make network calls (could be legitimate like Clawkeeper API calls)
- **PASS**: No suspicious patterns found
- Skip Clawkeeper's own scripts (they're expected to call clawkeeper.dev)
...
```
Clawkeeper Claude Code Audit — Grade: [LETTER]
...
For deeper dives: /clawkeeper:secrets (detailed credential scan) | /clawkeeper:inspect (detailed plugin analysis)
```
