---
name: login
description: Log in to Clawkeeper to link the plugin to your account. Supports interactive device-code browser flow (no args) or headless `--api-key <key>` for CI, managed laptops, and fleet provisioning. Run when the user wants to authenticate, link their account, set up an API key, or paste an existing key.
---

# Clawkeeper Login

You are helping the user log in the Clawkeeper plugin to their clawkeeper.dev account. There are two paths:
...
After either path succeeds, HTTP hooks are installed into `~/.claude/settings.json` so Claude Code natively sends hook events to the Clawkeeper API.

## Step 0: Parse Arguments

Check the user's slash command invocation for flags:

- If the invocation is `/clawkeeper-code:login --api-key <KEY>` or `/clawkeeper-code:login --api-key=<KEY>`: take the headless path (Step 1B). The `<KEY>` value is the raw API key string. **Never echo it.**
...
The --api-key flag requires a value. Usage:
  /clawkeeper-code:login --api-key sk_live_...

Generate a key at https://clawkeeper.dev/settings#api-keys
```
and stop.
...
1. Validate the key against the health endpoint. Display a brief message while the request is in flight (do NOT echo the key itself):
   ...
   HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 \
     "https://clawkeeper.dev/api/v1/claude-code/health" \
...
2. If the HTTP status is not `200`:
   ```
   That API key did not validate. clawkeeper.dev returned HTTP [status].
   ...
   Generate a fresh key at https://clawkeeper.dev/settings#api-keys
   ```
...
3. If the HTTP status is `200`, write the key and install hooks + register. Use the same Step 5 and Step 6 code blocks as the interactive path. To write the key, use a heredoc so the value never appears on the command line:
   ...
   printf '%s' "$API_KEY_FROM_FLAG" > "$CK_DIR/api_key"
   chmod 600 "$CK_DIR/api_key"
...
5. Display (never include the API key value):
   ...
   Machine ID generated at $CK_DIR/machine_id
...
**CRITICAL for Step 1B: Never echo the API key in any output, prompt, status message, or confirmation. Never include it in a log. The only legitimate destination for the key value is the `$CK_DIR/api_key` file (mode 600) and the Authorization header of outbound HTTPS requests.**
