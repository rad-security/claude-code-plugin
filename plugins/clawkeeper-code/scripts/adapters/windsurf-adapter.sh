#!/usr/bin/env bash
# windsurf-adapter.sh — Windsurf (Codeium Cascade) hook adapter for Clawkeeper
#
...
export CLAWKEEPER_IDE="windsurf"
...
# and logs per the org's log_all_tool_calls setting. Non-blocking.
{
  ...
  if [ -n "$API_KEY" ]; then
    ...
    printf '%s' "$INPUT" | curl -s --max-time 4 -X POST "https://clawkeeper.dev/api/v1/evaluate?tool=windsurf" \
...
if printf '%s' "$RESPONSE" | grep -q '"permissionDecision":"deny"'; then
  ...
  printf '%s\n' "${REASON:-Blocked by Clawkeeper}" >&2
  exit 2
fi
